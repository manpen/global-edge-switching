def return_boilerplate(jobid):
    boilerplate = "" \
        + "#!/bin/bash\n" \
        + "#SBATCH --job-name=acorr" + str(jobid) + "\n" \
...
with open("sorted.csv", "r") as reader:
    for line in reader.read().splitlines()[1::]:
        s = line.split(",")
        if int(s[2]) >= 100 and int(s[1]) <= 5000 and int(s[2]) <= 5000000:
...
for Jobgraphs, index in zip(J, range(len(J))):
    with open("job-{}.sh".format(index), "w") as writer:
        writer.write(return_boilerplate(index))
        if len(Jobgraphs):
            continue
        for graph in Jobgraphs:
            log_fn = graph[0].split("/")[-1].split(".simp-undir-edges")[0]
            writer.write("./autocorrelation_realworld 1 {}/{} 20 1 2 3 4 5 6 8 9 10 12 14 15 18 20 30 40 --minsnaps {} --maxsnaps {} --pus {} >> {}/$SLURM_JOB_ID/{}-1.log\n".format(inputdir, graph[0], minsnaps, maxsnaps, pus, outputdir, log_fn))
            writer.write("./autocorrelation_realworld 2 {}/{} 20 1 2 3 4 5 6 8 9 10 12 14 15 18 20 30 40 --minsnaps {} --maxsnaps {} --pus {} >> {}/$SLURM_JOB_ID/{}-2.log\n".format(inputdir, graph[0], minsnaps, maxsnaps, pus, outputdir, log_fn))
